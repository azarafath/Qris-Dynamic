...
async function createPosterPNG({ qrDataUrl, amount, note, link }: PosterOptions) {
...
export default function QrisGenerate() {
  ...
  async function onShare() {
    ...
    try {
      ...
      ctx.fillStyle = "#ffffff"
      ctx.font = "600 36px system-ui, -apple-system, Segoe UI, Roboto, Helvetica"
      ctx.fillText(formatIDR(Number(amount)), 120, 1132)

      // Link
      ctx.fillStyle = "#0F6FFF"
      ctx.font = "500 28px system-ui, -apple-system, Segoe UI, Roboto, Helvetica"
      wrapText(ctx, shareUrl, 100, 1240, width - 200, 34)
      ...
      // WhatsApp fallback (mobile) + desktop copy + download
